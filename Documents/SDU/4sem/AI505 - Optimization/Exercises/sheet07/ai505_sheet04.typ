...
= Exercise $2^*$
In a regression task we assume $h(x;w) = w_0+w_1 x_1+w_2 x_2+dots + w_d x_d$. For the esimation of the parameters $w in RR^(d+1)$ using the examples ${(x_1,y_1), dots (x_n,y_n)}$ we can use the least squares loss function
