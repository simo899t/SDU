...
// Backwards-compatible wrapper around the `curryst` package:
//   ptree(p1, p2, ..., conclusion: c, rule: r, titles: (t1, t2, ...))
// Premises can be nested ptree calls. `titles` stacks a label above each
// premise (curryst has no native equivalent).
#let ptree(..args, conclusion: none, rule: none, titles: none) = {
  let premises = args.pos()
  let decorated = if titles == none { premises } else {
    premises.enumerate().map(((i, p)) => {
      if i < titles.len() {
        align(center, stack(dir: ttb, spacing: 3pt, emph(titles.at(i)), p))
      } else { p }
    })
  }
  _curryst-prooftree(_curryst-rule(
    name: rule,
    ..decorated,
    conclusion,
  ))
