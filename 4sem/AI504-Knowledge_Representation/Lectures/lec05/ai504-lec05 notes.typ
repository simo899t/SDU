...
A #underline("proof tree") is a binary tree labeled by sentences such that at each non-leaf node
...
Suppose that $T$ is a proof tree. Then $T$ is a *proof* of $phi$ from $Gam$ if:
...
#definition(
  ...
  title: [Definition: $Gam prov phi$],
)

During this and the next lecture we want to prove
$ Gam ent phi <==> Gam prov phi $

where
$ underbrace(Gam ent phi ==> Gam prov phi, "Soundness" (ent "implies" prov)) $
and
$ underbrace(Gam ent phi <== Gam prov phi, "Completeness" (prov "implies" ent)) $

= Soundness

#definition(
  [For any set of sentences $Gam$ and sentence $phi$, if $Gam prov phi$ then $Gam ent phi$],
  title: [Definition: Soundness],
)

For all $Gam, phi$: if there exists a proof of $phi$ from $Gam$, then for every model $cal(M)$, if $cal(M) ent Gam$ then $cal(M) ent phi$.
